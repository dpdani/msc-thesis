...
We additionally enhance this scheme with other useful features for a concurrent context: synchronous operations, reservation buffers for the data table, and iteration partitioning.
...
The first claim should not be surprising (see for instance~\cite[\S13.1]{art-mp}).
...
Instead of looking up all the individual keys which are \emph{known} to be there, it may be that Python programs prefer to iterate over all keys.
And furthermore, it may be that when a program looks up one specific key in the dictionary, in most cases it really isn't known whether the key is there or not.
...
Our approach, detailed in the following Section~\ref{sec:lazy-robin-hood}, differs from~\cite{bolt} in that we don't necessarily constrain the hash table to this scheme: we maintain the Robin Hood invariants as long as we can do so in a lock-free manner.
