...
\begin{enumerate}
    ...
    \item a \emph{lookup} returns the value associated with a certain key.
\end{enumerate}
...
Examples of these paradigms are, except from obviously single-threaded programs include:
\begin{enumerate}
    ...
    \item multiprocessing-based concurrency, where the separate strands of execution are offloaded from a main CPython process to a set of child CPython processes, each having their own GIL\@.
\end{enumerate}
...
A plan to port this work to Python 3.13 is described in Section~\ref{subsec:compatibility-with-3.13-free-threading}.

What follows in the next Chapter, is an account of the free-threading-related changes to the CPython interpreter.
The ones described here seem to be fully implemented, with the remaining work for the next release being primarily focused on resolving numerous, minor issues.
