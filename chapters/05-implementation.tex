...
The operation begins by computing the $d_0$ (distance-0) index slot by calling the \texttt{Distance0Of} function, which internally re-hashes the CPython-generated hash, as described in Section~\ref{subsec:maier}, then it linearly probes the index for the searched key, and visits the data table accordingly, possibly pruning the search if the hash table is in a compact state.
